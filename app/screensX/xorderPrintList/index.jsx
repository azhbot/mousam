...
import XorderPrintListCard from "./components/xorderPrintListCard";
import { SafeAreaView } from "react-native-safe-area-context";
import { useDispatch, useSelector } from "react-redux";
import {
  selectCompayOrderInvoices,
  selectcompanyOrderInvoices,
...
const XorderPrintListScreen = () => {
  ...
  const companyOrderInvoices = useSelector(selectCompayOrderInvoices);
  ...
  const handleCardSelect = (v) => {
    ...
    setSelectedCards((prev) => {
      const updated = { ...prev };
      if (updated[v.entryId]) {
        delete updated[v.entryId];
      } else {
        updated[v.entryId] = v;
      }
  ...
  const renderItem = ({ item, index }) => (
    <XorderPrintListCard
      index={index}
      orderInvoice={item}
      isSelected={!!selectedCards[item.entryId]}
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.title}>
        <View style={styles.titleLeft}>
          ...
          <CustomLabel fontFamily="interMedium">Order Invoices</CustomLabel>
        </View>

        {/* ✅ Wrap dots icon with touchable + ref */}
        <TouchableOpacity onPress={handleDotsPress} ref={dotsRef}>
          <Icon name="dots-vertical" />
        </TouchableOpacity>
      </View>
      {companyOrderInvoices?.length === 0 ? (
        <View style={styles.empty}>
          <CustomLabel color={colors.gray}>No Order Invoices</CustomLabel>
        </View>
      ) : (
        <FlatList
          data={companyOrderInvoices || []}
          keyExtractor={(item, index) => index.toString()}
          renderItem={renderItem}
          contentContainerStyle={{ paddingBottom: 80 }} // Prevents last item from being cut off
          ListEmptyComponent={
            <View style={styles.empty}>
              <CustomLabel>No data</CustomLabel>
            </View>
          }
      ...
      <PopupOptions
        ...
        options={["Print", "Download", "Share", "Delete"]}
...
export default XorderPrintListScreen;
